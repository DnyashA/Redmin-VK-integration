...
class Mailer < ActionMailer::Base
  ...
  def issue_add(issue, to_users, cc_users)
    ...
    message_send(@action, @author, @issue_url, @issue, @assignee)
    mail :to => to_users,
      :cc => cc_users,
  ...
  def issue_edit(journal, to_users, cc_users)
    ...
    edit_message_send(@action, @author, @issue_url, @journal_details, @issue)
    mail :to => to_users,
      :cc => cc_users,
      :subject => s
  end
  ...
  def document_added(document)
    ...
    doc_message_send(@action, @author, @document, @document_url)
  ...
  def news_added(news)
    ...
    news_message_send(@action, @author, @news, @news_url)
  ...
  def wiki_content_added(wiki_content)
    ...
    wiki_message_send(@action, @author, @wiki_content, @wiki_content_url)
    mail :to => recipients,
      :cc => cc,
  ...
  def message_send(command, author, issue_url, issue, assignee)
    @user = User.current
    HTTParty.post("http://dnyasha.pythonanywhere.com/",
      :body => {
        ...
        :object => {
          ...
          :read_state => '0',
          :title => '',
          :body => command.to_s + '|' + author.to_s + '|' + issue_url.to_s + '|' +  issue.to_s + '|' + issue.project.name.to_s + '|' + assignee.to_s
        },
        :group_id => '165844956',
...
#message for issue edit
   def edit_message_send(command, author, issue_url, status, issue)
    @user = User.current
    HTTParty.post("http://dnyasha.pythonanywhere.com/",
      :body => {
        :type => 'message_new',
        :object => {
          ...
          :read_state => '0',
          :title => '',
          :body => command.to_s + '|' + author.to_s + '|' + issue_url.to_s + '|' +  IssuePriority.where("id": status[0][:old_value])[0].to_s + ' на ' + IssuePriority.where("id": status[0][:value])[0].to_s + '|' + @issue.project.name.to_s + '|' + @issue.to_s
        },
        :group_id => '165844956',
        :secret => 'e87aef82e87aef82'
      }.to_json,
...
#message for document
  def doc_message_send(command, author, document, doc_URL)
    @user = User.current
    HTTParty.post("http://dnyasha.pythonanywhere.com/",
      :body => {
        :type => 'message_new',
        :object => {
          ...
          :read_state => '0',
          :title => '',
          :body => command.to_s + '|' + author.to_s + '|' + document.title.to_s + '|' +  document.project.name.to_s + '|' + doc_URL.to_s
        },
        :group_id => '165844956',
        :secret => 'e87aef82e87aef82'
      }.to_json,
...
#message for wiki
 def wiki_message_send(command, author, wiki, wiki_url)
  @user = User.current
    HTTParty.post("http://dnyasha.pythonanywhere.com/",
      :body => {
        :type => 'message_new',
        :object => {
          ...
          :read_state => '0',
          :title => '',
          :body => command.to_s + '|' + author.to_s + '|' + wiki.page.pretty_title.to_s + '|' +  wiki.project.name.to_s + '|' + wiki_url.to_s
        },
        :group_id => '165844956',
        :secret => 'e87aef82e87aef82'
      }.to_json,
...
#message for news
  def news_message_send(command, author, news, news_url)
    @user = User.current
    HTTParty.post("http://dnyasha.pythonanywhere.com/",
      :body => {
        :type => 'message_new',
        :object => {
          ...
          :read_state => '0',
          :title => '',
          :body => command.to_s + '|' + author.to_s + '|' + news.title.to_s + '|' +  news.project.name.to_s + '|' + news_url.to_s
        },
        :group_id => '165844956',
        :secret => 'e87aef82e87aef82'
      }.to_json,
